...
public class PhaseDriver : MonoBehaviour
{
    ...
    { get { return modeDiff; } set { modeDiff = value; } }

    public bool IsSaved
    { get { return isSaved; } set { isSaved = value; } }

    // public GameObject PhaseObj
    // { get { return target_Obj; } private set { target_Obj = value; } }
    ...
    //     temp.modeDiff = modeDiff;
    //     temp.phaseDriver = this;
    //     // if (songData != null) temp.m_SongData = songData;
    //     linkedPhase.AddLast(temp);
    //     //페이즈 별 번호 추가
    //     if (!byDifficulty.ContainsKey(modeDiff))
    //     {
    //         byDifficulty.Add(modeDiff, linkedPhase.Count);
    //     }
    //     else
    //     {
    //         byDifficulty[modeDiff] = linkedPhase.Count;
    //     }
    //     temp.phaseNum = byDifficulty[modeDiff];
    //     //저장 델리게이트 등록
    ...
    // {
    // if (!resourceIO.Phase_Dic.ContainsKey(modeDiff)) return;
    // List<SongData> dataList = new List<SongData>();
    // dataList = resourceIO.Phase_Dic[modeDiff];
    // int temp = 0;
